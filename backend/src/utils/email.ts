...
const isConfigured = (v: string): boolean => !!v && !v.includes('your_email') && !v.includes('your_app_password');

export const sendPasswordResetEmail = async (
  email: string,
  resetLink: string
): Promise<void> => {
  const smtpConfigured = isConfigured(config.emailHost) && isConfigured(config.emailUser) && isConfigured(config.emailPass);
  if (!smtpConfigured) {
  ...
  try {
    ...
    logger.info(`[EMAIL_FALLBACK] Password reset link for ${email}: ${resetLink}`);
  }
};
